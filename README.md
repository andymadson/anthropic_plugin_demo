...
│   ├── skills/
│   │   ├── sql-review/SKILL.md            # user-invoked: /sql-review:sql-review
...
The guard reads each tool call as JSON on stdin, applies `policy.json`, and answers with a permission decision: `deny` blocks the call and tells Claude why, `ask` routes it to the human. Because it is a process gate rather than a prompt instruction, the result does not depend on model behavior. Same input, same decision, every run. The audit logger appends one JSON line per tool call, and the guard logs every denial, so the trail shows attempts and outcomes together.
...
```bash
claude --plugin-dir ./plugins/sql-review
```
...
- **Skills or hooks not showing after install:** restart Claude Code, or run `/reload-plugins`. Hook and agent changes do not hot-reload mid-session.
- **Policy edits not taking effect:** installed plugins run from a cache copy, not your checkout. Bump the version and update, or iterate with `claude --plugin-dir ./plugins/sql-review`.
- **Validate manifests:** `claude plugin validate .` for the marketplace, `claude plugin validate ./plugins/sql-review` for the plugin.
- **Windows:** the hook commands use `${CLAUDE_PLUGIN_ROOT}` shell substitution; run Claude Code under WSL or Git Bash, and clone into the WSL filesystem so SQLite can lock the database file.
