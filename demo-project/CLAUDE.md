...
Work in dev. The sql-review plugin enforces the team's guardrails (no prod targets, no destructive SQL, immutable migrations, no secrets in code) and writes an audit trail to `.claude/audit/`. Follow the `warehouse-conventions` skill for SQL standards.
